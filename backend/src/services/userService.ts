...
export const getTopPlayers = async (limit: number = 10) => {
    return await User.findAll({
        ...
        where: {
            guessedCount: { [Op.gt]: 0 } // Mostriamo solo chi ha indovinato almeno una parola
        },
        order: [
            ['guessedCount', 'DESC'],        // 1° Criterio: Parole indovinate 
            ['failedCount', 'ASC'],          // 2° Criterio (Spareggio): Meno fallimenti 
            ['totalAttemptsUsed', 'ASC']     // 3° Criterio (Spareggio): Meno tentativi usati 
        ],
        limit
    });
};
